...
fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Commands::Block(command) => match command {
            BlockCommands::Create { seed, signing_key, previous, transaction } => {
                let mut rng = match seed {
                    Some(seed) => ChaCha20Rng::seed_from_u64(seed),
                    None => ChaCha20Rng::from_entropy()
                };
                ...
                let mut transactions = Vec::with_capacity(transaction.len());
            ...
            BlockCommands::Approve { seed, signing_key, block } => {
                let mut rng = match seed {
                    Some(seed) => ChaCha20Rng::seed_from_u64(seed),
                    None => ChaCha20Rng::from_entropy()
                };
